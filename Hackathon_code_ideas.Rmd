    ...
    if(team_num == 24){
      ...
      for(i in seq(1,dim(team_sensor_data)[1])){
        ...
        if(is.na(as.numeric(date_i))==TRUE){
          ...
          dates = c(dates, as.POSIXct(as.Date(as.numeric(date_i), origin="1899-12-30")))
        }
      }
...
import_sensor_data = function(sensor_teams, sensor_url, sensor_start, sensor_cols, xlsx_TF, CROPX_TF){
  ...
  for(team in sensor_teams)
  {
    ...
    ts_weekly_data = weekly_sensor_averages(ts_daily_data)
...
weather_weekly = weekly_sensor_averages(mesonet_data)
...
```{r}
# Extract weekly data
all_data_sensors_weekly = all_data_sensors_moisture[all_data_sensors_moisture$Summarization_level=="Weekly_Average",]

# Merge with weather
weekly_merge1 = merge(all_data_sensors_weekly, weather_weekly, by="Timestamp", all=TRUE)
